...
function tl_Main() {
  ...
  tl.fromTo(
    ...
    { width: "3.5rem" },
    {
      ...
      duration: 0.3,
      onStart: () => {
        barMain.value.children[0].style.display = "none";
      },
      onComplete:()=>{
        barMain.value.children[0].style.display = "flex";
        barMain.value.children[0].style.opacity = "0";
      }
    }
  );
  tl.fromTo(barMain.value.children[0], { opacity: 0 }, { opacity: 1, duration: 0.3 });
}
...
<template>
  <div class="w-full h-24 sm:h-20 flex justify-between items-center pt-8 sm:py-8 pr-20 pl-10 sm:px-7">
    ...
    <div ref="barMain" class="w-2/5 bg-custom-Secondary-2 h-14 rounded-full text-minium sm:hidden">
      <div class="w-full h-full flex items-center justify-between px-14 gap-6">
